...
print("Logging and and finding provider...")
provider = IBMQ.load_account()
backends = provider.backends(filters=lambda x: x.configuration().n_qubits >= solver.num_qubits
    and not x.configuration().simulator
    and x.status().operational==True)
backend = least_busy(backends)
print("Selected backend: {}".format(backend.name()))
print("Executing circuit...")
res = execute(solver, backend=backend, shots=args.shots).result()
